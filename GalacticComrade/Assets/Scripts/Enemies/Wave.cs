...
public class Wave : MonoBehaviour {
    ...
    IEnumerator CreateBossWave() 
    {
        for (int i = 0; i < count; i++)
        {
            GameObject newBoss;
            newBoss = Instantiate(enemy, enemy.transform.position, Quaternion.identity);
            BossFollowThePath followComponent = newBoss.GetComponent<BossFollowThePath>();
            followComponent.path1 = pathPoints;
            followComponent.path2 = pathPoints2;
            followComponent.speed = speed;
            followComponent.rotationByPath = rotationByPath;
            followComponent.loop = Loop;
            followComponent.SetPath();
            Boss bossComponent = newBoss.GetComponent<Boss>();
            bossComponent.shotChance = shooting.shotChance;
            bossComponent.shotTimeMin = shooting.shotTimeMin;
            bossComponent.shotTimeMax = shooting.shotTimeMax;
            bossComponent.difficultyTimer_ = new Timer(1);

            // bossComponent.difficultyTimer_.Start();
